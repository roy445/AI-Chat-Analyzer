...
export function ErrorReportPage() {
  ...
  const submit = async (event: FormEvent) => { event.preventDefault(); setBusy(true); setStatus(""); try { const response = await fetch("/api/error-report", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ type, code: normalizedCode, message, page, email, clientId }) }); const data = await response.json() as { error?: string; message?: string; nextAllowedAt?: string }; if (!response.ok) throw new Error(data.nextAllowedAt ? `${data.error} 下次可提交時間：${new Date(data.nextAllowedAt).toLocaleString("zh-TW")}` : data.error || "送出失敗。"); setStatus(data.message || "已收到你的內容，謝謝你幫助我們改善。" ); setMessage(""); } catch (error) { setStatus(error instanceof Error ? error.message : "送出失敗，請稍後再試。" ); } finally { setBusy(false); } };
  return <SupportShell title="回報錯誤與提供回饋" eyebrow="Help us improve"><div className="support-card"><p>不知道錯誤代碼也沒關係。你可以選擇回報類型，照著提示自由描述；如果是系統錯誤，請把畫面上看到的代碼貼上，系統會自動判別名稱。</p><form className="support-form" onSubmit={submit}><label>你想提供什麼？<select value={type} onChange={(event) => setType(event.target.value as typeof type)}><option value="error">回報錯誤（不限次數）</option><option value="suggestion">提出功能建議（每 3 天一次）</option><option value="question">提出使用問題（每 3 天一次）</option></select><small className="field-help">錯誤回報不限次數；建議與使用問題為每個瀏覽器識別每 3 天一次。</small></label>{isError ? <><label>錯誤代碼（不知道可留白）<input value={code} onChange={(event) => setCode(event.target.value.toUpperCase())} placeholder="例如 AI-006；不知道就留白" /><small className="field-help">可輸入系統顯示的代碼；沒看過的代碼也可以直接貼上，或留白後用文字說明。</small></label><label>系統判別的錯誤名稱<input value={matched ? `${info.code}｜${info.name}` : normalizedCode ? "未收錄代碼：將由建立者人工確認" : "未提供代碼：請由下方描述判斷"} readOnly aria-readonly="true" className={matched ? "field-readonly matched" : "field-readonly"} /><small className="field-help">這一欄由系統自動產生，不能手動修改；未知代碼不會被誤分類。</small></label></> : <label>相關錯誤代碼（選填）<input value={code} onChange={(event) => setCode(event.target.value.toUpperCase())} placeholder="如果沒有錯誤代碼，可以留白" /></label>}<label>錯誤或回饋發生位置<select value={page} onChange={(event) => setPage(event.target.value)}><option value="home">首頁</option><option value="platform">選擇平台</option><option value="guide">匯出教學</option><option value="upload">上傳與檔案解析</option><option value="identify">確認聊天者</option><option value="report">分析報告</option><option value="ai">AI 深度分析</option><option value="share">建立或查看分享報告</option><option value="admin">管理員後台</option><option value="report-error">錯誤回報頁</option><option value="other">其他畫面</option></select><small className="field-help">這不是網址欄位；請選擇你看到問題或想提出建議的畫面。</small></label><label>聯絡信箱（選填）<input type="email" value={email} onChange={(event) => setEmail(event.target.value)} placeholder="you@example.com，方便我們回覆時再填" /></label><label>{type === "error" ? "請描述錯誤" : type === "suggestion" ? "請描述你的建議" : "請描述你遇到的問題"}<textarea value={message} onChange={(event) => setMessage(event.target.value)} placeholder={type === "error" ? "例如：我在分析報告按下 AI 深度分析後，看到什麼訊息？是否重試過？不知道代碼也可以直接描述。" : type === "suggestion" ? "例如：希望報告可以增加哪些功能？你會怎麼使用？" : "例如：你想完成什麼操作？卡在哪一步？畫面出現什麼？"} minLength={10} maxLength={3000} required /></label><button className="btn-primary" type="submit" disabled={busy}>{busy ? "正在送出…" : type === "error" ? "送出錯誤回報" : type === "suggestion" ? "送出功能建議" : "送出使用問題"} <Send size={15} /></button>{status && <div className="notice" role="status">{status}</div>}</form></div><div className="support-card"><h2>送出前提醒</h2><p>請勿貼上完整聊天內容、密碼、API key、DATABASE_URL 或其他私人資料。功能建議與使用問題為了避免重複垃圾訊息，每 3 天限送一次；真正的錯誤回報永遠可以再次送出。</p></div></SupportShell>;
}
