...
export function ErrorReportPage() {
  const [code, setCode] = useState(() => typeof window === "undefined" ? "" : new URLSearchParams(window.location.search).get("code") || ""); const [message, setMessage] = useState(""); const [page, setPage] = useState(() => typeof window === "undefined" ? "" : window.location.pathname); const [email, setEmail] = useState(""); const [status, setStatus] = useState(""); const [busy, setBusy] = useState(false);
  const submit = async (event: FormEvent) => { event.preventDefault(); setBusy(true); setStatus(""); try { const response = await fetch("/api/error-report", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ code, message, page, email }) }); const data = await response.json() as { error?: string; message?: string }; if (!response.ok) throw new Error(data.error || "回報送出失敗。"); setStatus(data.message || "已收到你的回報，謝謝你幫助我們改善。" ); setMessage(""); } catch (error) { setStatus(error instanceof Error ? error.message : "回報送出失敗，請稍後再試。" ); } finally { setBusy(false); } };
  return <SupportShell title="回報錯誤" eyebrow="Help us improve"><div className="support-card"><p>請提供錯誤代碼與簡短描述。表單不會自動附加原始聊天內容、API key 或資料庫連線字串。</p><form className="support-form" onSubmit={submit}><label>錯誤代碼<input value={code} onChange={(event) => setCode(event.target.value.toUpperCase())} placeholder="例如 AI-006" required /></label><label>發生頁面<input value={page} onChange={(event) => setPage(event.target.value)} placeholder="例如 /report" /></label><label>聯絡信箱（選填）<input type="email" value={email} onChange={(event) => setEmail(event.target.value)} placeholder="you@example.com" /></label><label>問題描述<textarea value={message} onChange={(event) => setMessage(event.target.value)} placeholder="請描述你做了什麼、看到了什麼，以及是否可以重現。" minLength={10} maxLength={2000} required /></label><button className="btn-primary" type="submit" disabled={busy}>{busy ? "正在送出…" : "送出錯誤回報"} <Send size={15} /></button>{status && <div className="notice" role="status">{status}</div>}</form></div><div className="support-card"><h2>隱私提醒</h2><p>請勿貼上完整聊天內容、密碼、API key、DATABASE_URL 或其他私人資料。錯誤回報只會寄送錯誤代碼、描述、頁面與必要的技術摘要。</p></div></SupportShell>;
}
